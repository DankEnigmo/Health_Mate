"""
Network Shell - Wrapper for base network and head networks
Adapted from openpifpaf v0.13.11
"""
...
class Shell(torch.nn.Module):
    """Shell wrapper combining base network and head networks."""
...
class CrossTalk(torch.nn.Module):
    """Cross-talk regularization between images in batch."""
    ...
    def forward(self, image_batch):
        if self.training and self.strength:
            ...
            image_batch += rolled_images * self.strength
        return image_batch


def model_defaults(net_cpu):
    """Apply default settings to model modules."""
